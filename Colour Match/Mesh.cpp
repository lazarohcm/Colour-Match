...
Mesh* Mesh::CreateSquare2D(Direct3D* direct3D, const char* identifier)
{
	...
	verts[3].colour = Color(1.0f, 1.0f, 1.0f, 1.0f);
...
Mesh* Mesh::CreateAbstractArt(Direct3D* direct3D, const char* identifier)
{
	...
	int indexCount = (WIDTH-1) * (HEIGHT-1) * 3;
	...
	if (!indices)
		return false;
	int currentVertex = 0;
	//Creating the vertices arrays
	for(int y = -5; y < HEIGHT - 5; y++) {
		for (int x = -5; x < WIDTH - 5; x++) {
			
			verts[currentVertex].position = Vector3(x, y, 0.0f);
			verts[currentVertex].colour = Color(0.1f, abs(y)*0.2f, abs(x)* 0.2f);
	...
	for (int y = 0; y < HEIGHT -1; y++) {
		for (int x = 0; x < WIDTH -1 ; x++) {
			...
			currentVertex = (y*WIDTH) + x;
			
			indices[currentIndex++] = currentVertex;
			indices[currentIndex++] = currentVertex + WIDTH;
			indices[currentIndex++] = currentVertex + (WIDTH + 1);
		}
	}
